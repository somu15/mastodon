...
//   params.addRequiredParam<RealVectorValue>(
//       "leverarm_direction", "Lever arm direction");
...
//     _normal_direction(isParamValid("normal_direction") ? &getParam<unsigned int>("normal_direction") : nullptr)
// {
//   if (_tensor && _p)
//     mooseError(
//         "In block ",
...
// {
//   RealVectorValue diff1 = _q_point[_qp] - _ref_point;
//   if (_tensor)
//   {
//     return _normals[_qp] * (MetaPhysicL::raw_value((*_tensor)[_qp]) * (*_stress_dir)) * diff1 * (*_leverarm_direction);
//     // return _normals[_qp] * (MetaPhysicL::raw_value((*_tensor)[_qp]) * (*_stress_dir)) * (diff1(0) * (*_leverarm_direction)(0) + diff1(1) * (*_leverarm_direction)(1) + diff1(2) * (*_leverarm_direction)(2));
//   }
//   else
//   {
//     if (_normal_direction)
//       return _normals[_qp]((*_normal_direction)) * ((*_p)[_qp]) * diff1 * (*_leverarm_direction);
//     else
//       return ((*_p)[_qp]) * diff1 * (*_leverarm_direction);
